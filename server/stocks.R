stocks_server <- function(input, output, session) {
  ...
  stocks_performance <- reactive({
    ...
    period_days <- switch(input$stocks_perf_period,
                          "1w" = 7,
                          "1m" = 30,
                          "3m" = 90,
                          "1y" = 365,
    ...
    if(period_days == Inf) {
      start_date <- input$date_range[1]
      end_date <- input$date_range[2]
      data_period <- get_close_data(input$Stocks, start_date, end_date)
    } else {
      start_date <- Sys.Date() - period_days
      end_date <- input$date_range[2]
      data <- get_close_data(input$Stocks, start_date, end_date)
  ...
  # Calculate largest changes for stocks
  ...
  output$biggest_stock_gain <- renderInfoBox({
    ...
    biggest_gain_stock <- which.max((changes$change_pct))
    biggest_gain <- changes[biggest_gain_stock, ]
  ...
  # Plot outputs
  output$stocks_market_performance_plot <- renderPlotly({
    data <- stocks_data()
    ...
    tryCatch({
      ...
      normalized_data <- normalize_data(data, input$stocks_norm_method)
      ...
      yaxis_title <- switch(input$stocks_norm_method,
  ...
  output$stocks_risk_return_plot <- renderPlotly({
    data <- get_close_data(input$Stocks, input$date_range[1], input$date_range[2])
  ...
  # Table output
  output$stocks_performance_table <- renderDT({
    datatable(stocks_performance(), options = list(
      pageLength = 15,
      scrollX = TRUE
    )) %>%
